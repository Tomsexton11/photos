...
int main( int argc, char** argv )
{
	...
	if (argc!=7) 
	{
		printf("prepare %of_picture_cropped size_of_train_picture image_name_prefix original_picture_new_size equalize_color num_photos_in_directory \nprepare 0.3 100 p 800 1 100\n");
		return 0;
	}
	...
	if (rep==NULL) return 0;
	 ...
	 struct dirent *ent;
	//int index=1;
	int index = atoi(argv[6])+1;




// ATTEMPTING TO AUTOMATE COUNTING NUMBER OF IMAGES OF 'TOM' IN DIRECTORY - TRY popen()
	...
	//int num_im;
	//scanf("%d",&num_im);
	//system("ls | grep \"tom\" | wc -l");
	//trace("images of Tom already in database");	

	//stringstream ss;
	//ss << num_im;
	//string snum_im = ss.str();
	//trace("how many? "+string(snum_im));
	...
	while ((ent=readdir(rep)) != NULL)
	{
	 	...
	 	if (nLen>4)
	 	if ((imageName[nLen-1]=='g')&&(imageName[nLen-2]=='p')&&(imageName[nLen-3]=='j')&&(imageName[0]=='z'))
	 	{
